...
/// </summary>
public class Go2walk1 : MonoBehaviour
{
        ...
        while(true)
        {
            if (ros2Unity.Ok())
            {
                ...
                if(tp>=2)
                {
                    if(walk)
                    {
                        for(int i=0;i<12;i++)qdes[i]=qsit[i]*kh+uff[i];
                    }
                    else 
                    {
    ...
    private void Subscribe()
    {
        while(true)
        {
            ...
            /*else
            {
                Thread.Sleep(100); // Wait a bit before checking again if ROS2 is not ready
            }
    ...
    void FixedUpdate()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Space))walk = !walk; // 翻转布尔值
