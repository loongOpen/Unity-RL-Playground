...
public class G1moeAgent : Agent
{
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        if (Input.GetKeyDown(KeyCode.Alpha0))moe=0;
        if (Input.GetKeyDown(KeyCode.Alpha1))moe=1;
        if (Input.GetKeyDown(KeyCode.Alpha2))moe=2;
        //if (Input.GetKeyDown(KeyCode.Alpha3))moe=3;
        //if(!train && moe==1 && arts[0].velocity.magnitude<=0.05f)moe=0;
        //if(!train && moe==0 && arts[0].velocity.magnitude>=1.6f)moe=1;

        if(!train)SetModel("gewu", policy[moe]);
    }
    ...
    void FixedUpdate()
    {
        ...
        if(keyboard && moe==0)
        {
            ...
            if(vr==0 && vd==0)
            {
                if(Input.GetKey(KeyCode.A))wr=Mathf.MoveTowards(wr, -1.8f, v);
                else if(Input.GetKey(KeyCode.D))wr=Mathf.MoveTowards(wr, 1.8f, v);
        ...
        {
            float v=0.03f;
            if(Input.GetKey(KeyCode.W))vr=Mathf.MoveTowards(vr, 4f, v);
            else if(Input.GetKey(KeyCode.S))vr=Mathf.MoveTowards(vr, -1f, v);
            else vr=Mathf.MoveTowards(vr, 0f, v);
        ...
        {
            float v=0.03f;
            if(Input.GetKey(KeyCode.W))vr=Mathf.MoveTowards(vr, 1f, v);
            else if(Input.GetKey(KeyCode.S))vr=Mathf.MoveTowards(vr, -1f, v);
            else vr=Mathf.MoveTowards(vr, 0f, v);
