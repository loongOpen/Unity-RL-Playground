...
public class GewuVR : MonoBehaviour
{
    ...
    [Tooltip("启用左手扳机键控制grasp/open")]
    public bool enableLeftHandTrigger = true;
    
    [Tooltip("启用右手扳机键控制grasp/open")]
    public bool enableRightHandTrigger = true;
    ...
    [Tooltip("夹爪刚度（值越大，移动越快）")]
    public float gripperStiffness = 5000f;
    
    [Tooltip("夹爪阻尼（值越小，移动越快，但可能不稳定）")]
    public float gripperDamping = 100f;
    ...
    private bool lastTriggerPressedLeft = false;
    private bool lastTriggerPressedRight = false;
    
    // 夹爪状态（true=grasp, false=open）
    private bool isGripperGrasping = false;
    ...
    void Start()
    {
        ...
        // 2秒后自动启用IK
        StartCoroutine(EnableIKAfterDelay());
    }
    ...
    /// 设置夹爪位置
    /// </summary>
    void SetGripperPosition(float leftPosition, float rightPosition)
    {
        // 设置左侧夹爪位置
        if (leftFinger != null)
        {
    ...
    /// 延迟2秒后自动启用IK
    /// </summary>
    private System.Collections.IEnumerator EnableIKAfterDelay()
    {
        // 等待2秒
        ...
        {
            enableIKToggle.isOn = true;
            Debug.Log("GewuVR: 自动启用IK");
        }
    }
    ...
    void CreateSliderForJoint(int jointIndex)
    {
        ...
        float targetAngleDeg = joint.jointPosition[0] * Mathf.Rad2Deg;
    ...
    {
        if (enableIKToggle != null && enableIKToggle.isOn != enabled)
        {
        ...
        for (int i = 0; i < Mathf.Min(sliders.Count, revoluteJoints.Count); i++)
        {
            if (sliders[i] != null && revoluteJoints[i] != null)
            {
                ...
                if (ikEnabled)
                {
                    ...
                    float currentAngleRad = revoluteJoints[i].jointPosition[0];
    ...
    /// <summary>
    /// 检测扳机键，点一下执行grasp，再点一下执行open（切换模式）
    /// </summary>
    void CheckGripperTrigger()
    {
        // 检测左手扳机键
        if (enableLeftHandTrigger)
        {
            bool isTriggerPressedLeft = false;
            if (leftHandDevice.isValid)
            {
                leftHandDevice.TryGetFeatureValue(CommonUsages.triggerButton, out isTriggerPressedLeft);
                
                // 检测按下瞬间（从false变为true）
                if (isTriggerPressedLeft && !lastTriggerPressedLeft)
                {
                    // 切换状态
                    isGripperGrasping = !isGripperGrasping;
                    
                    if (isGripperGrasping)
                    {
                        // 执行grasp
                        SetGripperPosition(graspAngle, graspAngle);
                        Debug.Log("GewuVR: 左手扳机键按下，执行grasp");
                    }
                    else
                    {
                        // 执行open
                        SetGripperPosition(openAngle, openAngle);
                        Debug.Log("GewuVR: 左手扳机键按下，执行open");
                    }
                }
                
                lastTriggerPressedLeft = isTriggerPressedLeft;
            }
        }
        
        // 检测右手扳机键
        if (enableRightHandTrigger)
        {
            bool isTriggerPressedRight = false;
            if (rightHandDevice.isValid)
            {
                rightHandDevice.TryGetFeatureValue(CommonUsages.triggerButton, out isTriggerPressedRight);
                
                // 检测按下瞬间（从false变为true）
                if (isTriggerPressedRight && !lastTriggerPressedRight)
                {
                    // 切换状态
                    isGripperGrasping = !isGripperGrasping;
                    ...
                    if (isGripperGrasping)
                    {
                        // 执行grasp
                        SetGripperPosition(graspAngle, graspAngle);
                        Debug.Log("GewuVR: 右手扳机键按下，执行grasp");
                    }
                    else
                    {
                        // 执行open
                        SetGripperPosition(openAngle, openAngle);
                        Debug.Log("GewuVR: 右手扳机键按下，执行open");
                    }
                }
                
                lastTriggerPressedRight = isTriggerPressedRight;
            }
        }
    }
}
