...
public class G1AutoNavigator : MonoBehaviour
{
    public G1opAgent agent; // 拖拽你的Go2Agent脚本
    ...
    void Start()
    {
        ...
        if (navAgent != null && robotBody != null)
        {
            navAgent.transform.position = robotBody.GetComponentInParent<Transform>().position;  // 设置NavMeshAgent的初始位置为机器人位置
        }
    ...
    void Update()
    {
        ...
        // 解除v1禁用，允许前进
        agent.vr = Mathf.Clamp(horizontalDist, -0.25f, 0.25f);//为什么速度上不去?
