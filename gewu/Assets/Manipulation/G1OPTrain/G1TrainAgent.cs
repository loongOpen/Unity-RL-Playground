...
public class G1TrainAgent : Agent
{
    ...
    public override void OnEpisodeBegin()
    {
        ...
        //if(train)
        {
            //vr=0;
            //wr=0;
            vr = Random.Range(-1.2f,1.2f);
            wr = Random.Range(-1f,1f);
            cr = Random.Range(0.1f,0.6f);

            //if(Random.Range(0,3)==1)vr=0;
            if(Random.Range(0,2)==1)wr=0;
            
            if(Mathf.Abs(vr)<0.5f)vr=0;
            if(Mathf.Abs(wr)<0.4f)wr=0;
        }
        //wr=0;
    }
