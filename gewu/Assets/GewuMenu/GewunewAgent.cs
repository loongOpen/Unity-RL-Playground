...
public class GewunewAgent : Agent
{
    int tp = 0;
    int tq = 0;
    int tt = 0;
    ...
    void Start()
    {
        ...
        if (train && !_isClone) 
        {
            for (int i = 1; i < 14; i++)
            {
                ...
                clone.GetComponent<GewunewAgent>()._isClone = true; 
            }
        }
    }
    ...
    public override void OnEpisodeBegin()
    {
        ...
        if (fixbody)
        {
            ...
            // 当选中Fixbody后自动将物体抬高0.2
            Vector3 elevatedPos = pos0 + Vector3.up * 0.2f;
            arts[0].TeleportRoot(elevatedPos, rot0);
        }
        if (!fixbody)
        {
